...
Network = () ->
  ...
  setupData = (data) ->
    ...
    # countExtent = d3.extent(data.nodes, (d) -> d.match)
    # circleRadius = d3.scale.linear().range([3, 12]).domain(countExtent)
  ...
  updateNodes = () ->
    ...
    node.on("mouseover", showDetails)
      ...
      .on("click", (d) -> window.location = "fingerPrint/"+d.link)
  ...
  showDetails = (d,i) ->
    ...
    if d.type == 'author'
      curLinksData.forEach (k) -> 
        if k.source.id == d.id
          content += '<hr class="tooltip-hr">'
          content += '<p class="main">' + k.target.name + ': ' + k.prob + '</span></p>'
    if d.type == 'topic'
      curLinksData.forEach (k) ->
        if k.source.id == d.id
          content += '<hr class="tooltip-hr">'
...
$ ->
  ...
  d3.json "data/AT-network.json", (json) ->
    myNetwork("#vis", json)
